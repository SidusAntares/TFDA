...
def _calc_metrics_pretrain(src_pred, src_true, trg_pred, trg_true, log_dir, home_path, target_names):
    ...
    src_rep = classification_report(src_true_labels, src_pred_labels, target_names=target_names, digits=6,
                                    output_dict=True)
    trg_rep = classification_report(trg_true_labels, trg_pred_labels, target_names=target_names, digits=6,
                                    output_dict=True)
...
def _plot_umap(model, src_dl, trg_dl, device, save_dir, hparams, epoch):
    ...
    plt.scatter(src_dominant_embedding[:, 0], src_dominant_embedding[:, 1], c='lightsalmon', s=30,
                label="Source Dominant", marker='o')
    plt.scatter(trg_dominant_embedding[:, 0], trg_dominant_embedding[:, 1], c='aquamarine', s=30,
                label="Target Dominant", marker='x')
